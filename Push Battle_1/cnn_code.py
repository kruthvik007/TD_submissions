...
# CNN architecture for Q-learning
...
class DDQNAgent:
    def __init__(self, player=PLAYER1, gamma=0.99, lr=0.001, epsilon=1.0, epsilon_decay=0.995, epsilon_min=0.1):
        ...
        input_channels = 1  # Single channel (board state)
        output_dim = BOARD_SIZE * BOARD_SIZE * 2  # Potential actions

        # Initialize policy and target networks
        self.policy_net = CNNQNetwork(output_dim)
        self.target_net = CNNQNetwork(output_dim)
    ...
    def choose_action(self, game):
        ...
        if random.random() < self.epsilon:
            ...
            state = game.board[np.newaxis, np.newaxis, :, :]  # Reshape for CNN
    ...
    def replay(self):
        ...
        states = torch.tensor(np.array(states), dtype=torch.float32).unsqueeze(1)  # Add channel dimension for CNN
